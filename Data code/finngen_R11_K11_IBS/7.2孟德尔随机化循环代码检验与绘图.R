...
FileNames <- list.files(path="exposure/", pattern = "*.csv")
FileNames

outcomefile="./out/finngen_R10_M13_ANKYLOSPON_STRICT.csv"
exp_dat <- list() 
ex_pore <- c()
for(i in c(1:length(FileNames))){
  IV <- fread(paste0("exposure","/",FileNames[i]))
...
GWAS_2 <- subset(GWAS_1,GWAS_1$SNP %in% allSNP$SNP)  
rm(GWAS_1)

GWAS_2$PHENO<-"Ankylosing spondylitis" 
...
out_dat <- list() 
out_dat[[1]] <- out_data
out_come<-c("Ankylosing spondylitis") 
